...
export function Timer({ timeEntry, onStop }: TimerProps) {
  ...
  useEffect(() => {
    ...
    const initialSeconds = differenceInSeconds(
      new Date(),
      parseISO(timeEntry.started_at)
    );
  ...
  return (
    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
      <div className="flex-1">
        <div className="font-medium mb-1">{timeEntry.description}</div>
        <div className="flex flex-wrap gap-2 text-sm">
          {timeEntry.project && (
            <span className="text-muted-foreground">{timeEntry.project.name}</span>
          )}
          {timeEntry.task && (
            <>
      ...
      <div className="flex items-center gap-3">
        <div className="text-xl font-mono font-semibold w-24 text-center">
          {formatTime(seconds)}
        </div>
